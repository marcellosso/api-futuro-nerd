...
class Materia{
	...
	public function listBySerieId($id_serie){
		return $this->pdo->query("SELECT m.* FROM {$this->_table_name} m JOIN questoes q ON m.id = q.id_materia WHERE q.id_serie = ". $id_serie ." GROUP BY m.id, m.materia ORDER BY m.materia ASC")->fetchAll(\PDO::FETCH_ASSOC);
	}
